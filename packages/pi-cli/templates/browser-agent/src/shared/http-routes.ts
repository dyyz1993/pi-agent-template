...
import type { SseHandler, SseRouteHandlers } from "../gateway/sse-transport";
...
export function createHttpHandler(
	...
	const sseRoutes = sse as SseHandler & SseRouteHandlers;
	...
	return async (req, res) => {
		...
		if (url.pathname === "/api/events" && req.method === "GET") {
			...
			sseRoutes.handleSseConnect(req, res);
			return;
		}
		...
		if (url.pathname === "/api/rpc" && req.method === "POST") {
			...
			await sseRoutes.handleRpcPost(req, res);
			return;
		}
