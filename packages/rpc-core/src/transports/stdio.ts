...
export interface StdioTransportOptions {
	...
	stdin?:
		| NodeJS.ReadStream
		| { on: (event: string, handler: (...args: unknown[]) => void) => void; resume: () => void };
...
export class StdioTransport implements Transport {
	...
	private stdinListeners: Array<[string, (...args: unknown[]) => void]> = [];
	...
	async connect(): Promise<void> {
		...
		this.addStdinListener("data", (chunk: unknown) => {
			this.buffer +=
		...
		this.addStdinListener("error", (err: unknown) => {
	...
	private addStdinListener(event: string, handler: (...args: unknown[]) => void): void {
		(this.stdin as { on: (event: string, handler: (...args: unknown[]) => void) => void }).on(
			event,
			handler
		);
		this.stdinListeners.push([event, handler]);
	}

	private removeStdinListeners(): void {
		const offable = this.stdin as
			| { off?: (event: string, handler: (...args: unknown[]) => void) => void }
			| undefined;
		if (offable?.off) {
			for (const [event, handler] of this.stdinListeners) {
				offable.off(event, handler);
			}
		}
		this.stdinListeners = [];
	}
	...
	static createPair(options?: StdioTransportOptions): {
