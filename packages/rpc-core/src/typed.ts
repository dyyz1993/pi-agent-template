...
import type { EventPayload, EventMetadata, RPCCallOptions } from "./core/types";
...
export function createTypedClient<Methods, Events>(
	...
	return {
		call: <K extends keyof Methods>(
			...
			return client.call<MethodResult<Methods, K>>(method as string, params, callOptions);
		},
		...
		subscribe: <K extends keyof Events>(
			...
			return client.subscribe(
				...
				(e: RPCEvent) => {
					handler(
						e.payload as EventPayloadType<Events, K>,
...
export interface RPCClientAPI {
	call(method: string, params: unknown, options?: RPCCallOptions): Promise<unknown>;
	subscribe(
		event: string,
		handler: (payload: unknown, metadata: unknown) => void,
...
export interface RPCModule<
	Methods extends AnyMethods = AnyMethods,
