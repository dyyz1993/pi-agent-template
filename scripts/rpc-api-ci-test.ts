...
async function runTests(templateType: string, client: RPCClient): Promise<TestResult[]> {
	...
	// Common tests (all templates)
	{
		...
		try {
			const r = await client.call<{ pong: boolean; platform: string; timestamp: number }>(
				"system.ping",
				{}
			);
		...
		// feed.list
		{
			...
			try {
				...
				if (Array.isArray(r.posts)) {
					pass(name, `${r.posts.length} posts`);
				} else {
					fail(name, JSON.stringify(r));
				}
			} catch (e) {
	...
	if (templateType === "agent") {
		...
		// bash.execute
		{
			...
			try {
				...
				bashPid = r.pid;
				if (r.output?.includes("ci-test")) {
					pass(name, `pid=${r.pid}, output contains "ci-test"`);
				} else {
					fail(name, JSON.stringify(r));
				}
			} catch (e) {
		...
		// bash.listProcesses
		{
			...
			try {
				...
				if (Array.isArray(r.processes)) {
					pass(name, `${r.processes.length} processes`);
				} else {
					fail(name, JSON.stringify(r));
				}
			} catch (e) {
		...
		// rules.list
		{
			...
			try {
				...
				if (Array.isArray(r.rules)) {
					pass(name, `${r.rules.length} rules`);
				} else {
					fail(name, JSON.stringify(r));
				}
			} catch (e) {
		...
		if (ruleId) {
			...
			try {
				...
				if (typeof r.rule?.enabled === "boolean") {
					pass(name, `enabled=${r.rule.enabled}`);
				} else {
					fail(name, JSON.stringify(r));
				}
			} catch (e) {
		...
		// todo.add
		{
			...
			try {
				...
				todoId = r.item?.id;
				if (r.item?.content === "CI test task") {
					pass(name, `id=${todoId}`);
				} else {
					fail(name, JSON.stringify(r));
				}
		...
		// todo.list
		{
			...
			try {
				...
				if (Array.isArray(r.items)) {
					pass(name, `${r.items.length} items`);
				} else {
					fail(name, JSON.stringify(r));
				}
			} catch (e) {
		...
		// git.diff
		{
			const name = "git.diff";
			try {
				const r = await client.call<{ changes: unknown[]; diff?: string }>("git.diff", {
					repoPath: projectDir,
				});
				if (Array.isArray(r.changes) || typeof r.diff === "string") {
					pass(name, "diff retrieved");
				} else {
					fail(name, JSON.stringify(r));
				}
			} catch (e) {
		...
		// git.log
		{
			...
			try {
				...
				if (Array.isArray(r.commits)) {
					pass(name, `${r.commits.length} commits`);
				} else {
					fail(name, JSON.stringify(r));
				}
			} catch (e) {
		...
		// file.readFile
		{
			...
			try {
				...
				if (typeof r.content === "string") {
					pass(name, "file read successfully");
				} else {
					fail(name, JSON.stringify(r));
				}
			} catch (e) {
