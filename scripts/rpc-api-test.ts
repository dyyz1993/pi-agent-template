...
const TEMPLATES: TemplateConfig[] = [
  {
    ...
    methods: [
      "system.ping", "system.hello", "system.echo",
      "chat.list", "chat.send",
      "feed.post", "feed.list",
      "git.status",
      "file.listDir",
  ...
  },
  {
    ...
    methods: [
      "system.ping", "system.hello", "system.echo",
      "chat.list", "chat.send",
      "bash.execute", "bash.kill", "bash.listProcesses",
      "rules.list", "rules.add", "rules.toggle", "rules.remove",
      "todo.list", "todo.add", "todo.update", "todo.remove",
      "feed.post", "feed.list",
      "git.status", "git.diff", "git.log",
      "file.listDir", "file.readFile",
    ],
  },
];
...
async function createClient(port: number, token: string): Promise<{ client: RPCClient; transport: WebSocketTransport }> {
...
async function testTemplate(config: TemplateConfig): Promise<TestResult[]> {
  ...
  // Health
  {
    ...
    try {
      const res = await fetch(`http://localhost:${port}/health`);
      const body = (await res.json()) as Record<string, unknown>;
      if (body.status === "ok") { pass(label, `{ status: "ok" }`); } else { fail(label, JSON.stringify(body)); }
    } catch (e) {
  ...
  // Auth rejection
  {
    ...
    try {
      ...
      if (res.status === 401) { pass(label, "401 Unauthorized"); } else { fail(label, `got ${res.status}`); }
    } catch (e) {
      fail(label, (e as Error).message);
    }
  }
  ...
  // WS auth rejection
  {
    ...
    const authResult = await new Promise<string>((resolve) => {
      ...
      ws.onclose = (ev: CloseEvent) => { clearTimeout(timer); resolve(`closed:${ev.code}`); };
      ws.onerror = () => { clearTimeout(timer); resolve("error"); };
    });
    if (authResult === "closed:4001") { pass(label, "closed:4001"); } else { fail(label, authResult); }
  }
  ...
  try {
    ...
    // system.ping
    {
      ...
      try {
        ...
        if (r.pong === true && typeof r.timestamp === "number") { pass(label, `{ pong: true, timestamp: ${r.timestamp} }`); } else { fail(label, JSON.stringify(r)); }
    ...
    // system.hello
    {
      ...
      try {
        ...
        if (r.message === "Hello TestBot!") { pass(label, `"${r.message}"`); } else { fail(label, JSON.stringify(r)); }
      } catch (e) { fail(label, (e as Error).message); }
    }
    ...
    // system.echo
    {
      ...
      try {
        ...
        if (JSON.stringify(r) === JSON.stringify(payload)) { pass(label, "payload echoed"); } else { fail(label, `mismatch: ${JSON.stringify(r)}`); }
      } catch (e) { fail(label, (e as Error).message); }
    }
    ...
    if (config.methods.includes("chat.send")) {
      ...
      try {
        ...
        if (r.ok === true) { pass(label1, `{ ok: true }`); } else { fail(label1, JSON.stringify(r)); }
      } catch (e) { fail(label1, (e as Error).message); }

      await sleep(200);

      try {
        const r = await client.call<{ messages: unknown[]; hasMore: boolean }>("chat.list", { limit: 10 });
        if (Array.isArray(r.messages) && typeof r.hasMore === "boolean") { pass(label2, `${r.messages.length} messages, hasMore=${r.hasMore}`); } else { fail(label2, JSON.stringify(r)); }
      } catch (e) { fail(label2, (e as Error).message); }
    }
    ...
    if (config.methods.includes("timer.start")) {
      ...
      try {
        ...
        timerId = (r.timer as Record<string, unknown>)?.id as string | undefined
          || (r.id as string | undefined);
        if (r.started === true || (r.timer as Record<string, unknown>)?.status === "running") { pass(label1, timerId ? `id=${timerId}` : `started=true`); } else { fail(label1, JSON.stringify(r)); }
      } catch (e) { fail(label1, (e as Error).message); }
      ...
      if (timerId && config.methods.includes("timer.stop")) {
        try {
          ...
          if (stopped) { pass(label2, `stopped ${timerId}`); } else { fail(label2, JSON.stringify(r)); }
        } catch (e) { fail(label2, (e as Error).message); }
      }
    }
    ...
    if (config.methods.includes("bash.execute")) {
      ...
      try {
        const r = await client.call<{ pid: number; output: string }>("bash.execute", { command: "echo rpc-test" });
        bashPid = r.pid;
        if (r.output?.includes("rpc-test")) { pass(label1, `pid=${r.pid}, output contains "rpc-test"`); } else { fail(label1, JSON.stringify(r)); }
      } catch (e) { fail(label1, (e as Error).message); }

      try {
        const r = await client.call<{ processes: unknown[] }>("bash.listProcesses", {});
        if (Array.isArray(r.processes)) { pass(label2, `${r.processes.length} processes`); } else { fail(label2, JSON.stringify(r)); }
      } catch (e) { fail(label2, (e as Error).message); }

      if (bashPid) {
        try {
          const r = await client.call<{ success: boolean }>("bash.kill", { pid: bashPid });
          if (r.success === true) { pass(`[${name}] bash.kill`, `killed ${bashPid}`); } else { fail(`[${name}] bash.kill`, JSON.stringify(r)); }
        } catch (e) { fail(`[${name}] bash.kill`, (e as Error).message); }
      }
    }
    ...
    if (config.methods.includes("rules.add")) {
      ...
      try {
        ...
        ruleId = r.rule?.id;
        if (r.rule?.name === "RPC Test Rule") { pass(label1, `id=${ruleId}, name="${r.rule.name}"`); } else { fail(label1, JSON.stringify(r)); }
      } catch (e) { fail(label1, (e as Error).message); }

      try {
        const r = await client.call<{ rules: unknown[] }>("rules.list", {});
        if (Array.isArray(r.rules)) { pass(label2, `${r.rules.length} rules`); } else { fail(label2, JSON.stringify(r)); }
      } catch (e) { fail(label2, (e as Error).message); }

      if (ruleId) {
        try {
          const r = await client.call<{ rule: { id: string; enabled: boolean } }>("rules.toggle", { id: ruleId });
          if (typeof r.rule?.enabled === "boolean") { pass(label3, `enabled=${r.rule.enabled}`); } else { fail(label3, JSON.stringify(r)); }
        } catch (e) { fail(label3, (e as Error).message); }

        try {
          const r = await client.call<{ success: boolean }>("rules.remove", { id: ruleId });
          if (r.success === true) { pass(label4, `removed ${ruleId}`); } else { fail(label4, JSON.stringify(r)); }
        } catch (e) { fail(label4, (e as Error).message); }
      }
    }
    ...
    if (config.methods.includes("todo.add")) {
      ...
      try {
        ...
        todoId = r.item?.id;
        if (r.item?.content === "RPC test task") { pass(label1, `id=${todoId}`); } else { fail(label1, JSON.stringify(r)); }
      } catch (e) { fail(label1, (e as Error).message); }

      try {
        const r = await client.call<{ items: unknown[] }>("todo.list", {});
        if (Array.isArray(r.items)) { pass(label2, `${r.items.length} items`); } else { fail(label2, JSON.stringify(r)); }
      } catch (e) { fail(label2, (e as Error).message); }
      ...
      if (todoId) {
        try {
          ...
          if (r.item?.status === "completed") { pass(label3, `status=completed`); } else { fail(label3, JSON.stringify(r)); }
        } catch (e) { fail(label3, (e as Error).message); }

        try {
          const r = await client.call<{ success: boolean }>("todo.remove", { id: todoId });
          if (r.success === true) { pass(label4, `removed ${todoId}`); } else { fail(label4, JSON.stringify(r)); }
        } catch (e) { fail(label4, (e as Error).message); }
      }
    }
    ...
    if (config.methods.includes("feed.post")) {
      ...
      try {
        ...
        if (r.id) { pass(label1, `id=${r.id}`); } else { fail(label1, JSON.stringify(r)); }
      } catch (e) { fail(label1, (e as Error).message); }

      try {
        const r = await client.call<{ posts: unknown[] }>("feed.list", {});
        if (Array.isArray(r.posts)) { pass(label2, `${r.posts.length} posts`); } else { fail(label2, JSON.stringify(r)); }
      } catch (e) { fail(label2, (e as Error).message); }
      ...
      if (config.methods.includes("feed.subscribe")) {
        try {
          ...
          if (subId) { pass(`[${name}] feed.subscribe`, `id=${subId}`); } else { fail(`[${name}] feed.subscribe`, JSON.stringify(r)); }
        } catch (e) { fail(`[${name}] feed.subscribe`, (e as Error).message); }

        if (subId) {
          try {
            const r = await client.call<{ success: boolean }>("feed.unsubscribe", { subscriptionId: subId });
            if (r.success === true) { pass(`[${name}] feed.unsubscribe`, "success"); } else { fail(`[${name}] feed.unsubscribe`, JSON.stringify(r)); }
          } catch (e) { fail(`[${name}] feed.unsubscribe`, (e as Error).message); }
        }
      }
    }
    ...
    if (config.methods.includes("git.status")) {
      try {
        ...
        if (typeof r.branch === "string") { pass(`[${name}] git.status`, `branch=${r.branch}`); } else { fail(`[${name}] git.status`, JSON.stringify(r)); }
      } catch (e) { fail(`[${name}] git.status`, (e as Error).message); }
    }
    ...
    if (config.methods.includes("file.listDir")) {
      try {
        ...
        if (Array.isArray(r.entries)) { pass(`[${name}] file.listDir`, `${r.entries.length} entries`); } else { fail(`[${name}] file.listDir`, JSON.stringify(r)); }
      } catch (e) { fail(`[${name}] file.listDir`, (e as Error).message); }
    }
    ...
    if (config.methods.includes("search.search")) {
      try {
        ...
        if (typeof r.totalMatches === "number") { pass(`[${name}] search.search`, `${r.totalMatches} matches`); } else { fail(`[${name}] search.search`, JSON.stringify(r)); }
      } catch (e) { fail(`[${name}] search.search`, (e as Error).message); }
    }
