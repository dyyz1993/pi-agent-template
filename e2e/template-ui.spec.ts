...
test.describe("Template UI Smoke Tests", () => {
	...
	test("shows connecting spinner or main UI", async ({ page }) => {
		...
		await expect(body).toContainText(/Connecting|Messages|Start a conversation|Pi Agent|Pi Chat/i, {
			timeout: 10_000,
		});
	});
...
(process.env.TEMPLATE_TYPE === "agent" ? test : test.skip)(
	"should display center tab navigation",
	async ({ page }) => {
		await waitForAppReady(page);
