...
function getBackendPort(): number {
  if (existsSync(PORT_FILE)) {
    try { return parseInt(readFileSync(PORT_FILE, "utf-8").trim()); } catch { /* fall through */ }
  }
...
export default defineConfig({
  ...
  server: {
    ...
    proxy: {
      ...
      "/file": { target: `http://localhost:${backendPort}`, changeOrigin: true },
      "/upload": { target: `http://localhost:${backendPort}`, changeOrigin: true },
      "/api": { target: `http://localhost:${backendPort}`, changeOrigin: true, ws: true },
